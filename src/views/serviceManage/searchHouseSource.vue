    ...
    <div class="app-container">
        <div class="model-search clearfix">
            <el-select size="small" v-model="formData.cityId" 
                class="item-select" style="width: 150px;" clearable @change="getAreaList">
                <el-option
            ...
            <el-input size="small" v-model="formData.keyword" placeholder="请输入小区名或地址" 
                class="filter-item" style="width:300px;" prefix-icon="el-icon-search" clearable
        ...
        <div class="model-itemFilters">
            <div class="position clearfix">
                <dl class="clearfix">
                    <dt>区域<span>|</span></dt>
                    <dd>
                        <span :class="{active: isAreaActive[0].index == -1}" 
                            @click="selectAreaMethod(0,-1)">
                            不限
                        </span>
                        <span v-for="(item,index) in areaList"
                            :class="{active:index == isAreaActive[0].index}"
                            :key="index"
                            @click="selectAreaMethod(0,index,item.areaId)">
                            {{item.areaName}}
                        </span>
                    </dd>
                </dl>
                <div class="zoneItems">
                    <p style="display: inline-block; margin: 0" v-for="(value, key, index) in zoneList">
                        {{key}}
                        <span v-for="(item, k) in value"
                            :class="{active: key == isAreaActive[1].value && k == isAreaActive[1].index}"
                            :key="k"
                            @click="selectAreaMethod(1,k,item.zoneId,key)">
                            {{item.zoneName}}
                        </span>
                    </p>
                </div>
            ...
            <div class="list-more" >
                <div class="list-more-item clearfix"
                    v-for="(item, index) in itemFilters"
                    :key="index"
                    :label="item.label"
                    :value="item.value">
                    <dl>
                        ...
                        <dd>
                            ...
                            <span v-for="(v,k) in item.filters" 
                                :class="{active:k == isActive[index]}"
                                :key="k"
                                @click="selectMethod(index,k)">
                                {{v.label}}
                            </span>
        ...
        <div class="renterInfo" style="padding-top: 5px;">
            <el-form :inline="true" ref="ruleForm" :model="renterInfo" :rules="rules" size="small">
                ...
                <el-input type="textarea" placeholder="请输入租客详细需求" 
                    v-model="renterInfo.detailRequireMent"
                    style="margin-bottom: 15px; display: block;width: 688px;">
                </el-input>
            </el-form>
        </div>
