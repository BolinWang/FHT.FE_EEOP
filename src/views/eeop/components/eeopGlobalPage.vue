<template>
  <div class="app-container">
      ...
      <el-table :data="tableData" v-loading.body="listLoading" :max-height="tableHeight" size="small" fit stripe highlight-current-row>
        ...
        <el-table-column v-for="(item,index) in colModels" :label="item.label" :width="item.width" :key="index" fit show-overflow-tooltip>
          <template slot-scope="scope">
            ...
            <a v-else-if="item.type === 'link'" :href="scope.row[item.prop]" target="_blank" style="color:#409EFF">
              {{scope.row[item.prop]}}
            </a>
    ...
    <div class="dialog-info">
      <el-dialog :title="textMap[dialogStatus] + eeopTypeStr" :visible.sync="layer_showInfo" width="600px" @close="dialogClose">
        <el-form size="small" status-icon :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="100px" style='width: 500px; margin-left:20px;'>
          ...
          <el-form-item label="链接" prop="linkUrl">
            <el-autocomplete ref="autofocusElm" style="width: 100%" v-model="temp.linkUrl" :fetch-suggestions="querySearch" placeholder="请输入有效链接">
            </el-autocomplete>
          </el-form-item>
          <el-form-item label="上线时间" prop="effectiveTime">
            <el-date-picker v-model="temp.effectiveTime" :disabled="(temp[this.colModels[0].prop] && temp[this.colModels[0].prop] != 1) || nowOnline" type="datetime" placeholder="选择上线时间">
            </el-date-picker>
            <el-checkbox v-model="nowOnline" class="filter-item" v-if="temp[this.colModels[0].prop] != 2">立即上线</el-checkbox>
          </el-form-item>
          <el-form-item v-if="eeopType != 'interview'" label="下线时间" prop="ineffectiveTime">
            <el-date-picker v-model="temp.ineffectiveTime" :disabled="temp[this.colModels[0].prop] && temp[this.colModels[0].prop] != 1" type="datetime" placeholder="选择下线时间">
            </el-date-picker>
            <el-checkbox v-model="nowOffline" class="filter-item" v-if="temp[this.colModels[0].prop] == 2 || nowOffline">立即下线</el-checkbox>
          </el-form-item>
          <el-form-item label="图片" prop="picUrl">
            <el-upload :action="`${actionBaseUrl}/util/upload/uploadPicture`" :before-upload="pictureUpload" :on-preview="picturePreview" :on-remove="(file, fileList)=>{
                                  return pictureRemove(file, fileList, `picUrl`)
                              }" :on-success="(response, file, fileList)=>{
                                  return pictureSuccess(response, file, fileList, `picUrl`)
                              }" :on-change="resetFile" :on-error="pictureError" :file-list="fileList" accept="image/jpg,image/jpeg,image/png" list-type="picture">
          ...
          <el-form-item v-if="eeopType == 'banner'" label="缩略图" prop="thumbnail">
            <el-upload :action="`${actionBaseUrl}/util/upload/uploadPicture`" :before-upload="pictureUpload" :on-preview="picturePreview" :on-remove="(file, fileList)=>{
                              ...
                              }" :on-change="resetFile" :on-error="pictureError" :file-list="thumFileList" accept="image/jpg,image/jpeg,image/png" list-type="picture">
              <el-button size="small" type="primary" icon="el-icon-upload">{{btnText}}</el-button>
              <div slot="tip" class="el-upload__tip">请上传350 * 350的jpg/png图片，且不超过500kb</div>
            </el-upload>
          </el-form-item>
          <el-form-item label="简介" prop="introduction">
            <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}" placeholder="请输入简介" v-model="temp.introduction" :maxlength="50" style="position: relative;">
            </el-input>
